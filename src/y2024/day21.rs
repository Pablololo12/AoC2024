...
fn where_robot(to_enter: String, numpads: usize) -> usize {
    ...
    for c in to_enter.chars() {
        result += where_numpad(current, c, numpads, false);
        current = c
    }
    result
}
...
#[cached]
fn where_numpad(prev_pos: char, new_pos: char, numpads: usize, keys: bool) -> usize {
    let num_pad = match keys {
        true => vec!['7', '8', '9', '4', '5', '6', '1', '2', '3', 'C', '0', 'A'],
        _ => vec!['C', '^', 'A', '<', 'v', '>'],
    };
    ...
    while let Some((sc, typed)) = open.pop_front() {
        ...
        if keys && sc.0 == 3 && sc.1 == 0 {
            continue;
        } else if !keys && sc.0 == 0 && sc.1 == 0 {
            continue;
        } else {
            if sc.0 < np.0 {
...
pub fn run(inp: Vec<String>) -> (String, String) {
    ...
    for line in inp {
        ...
        let mut result = 0;
        for c in line.chars() {
            result += where_numpad(current, c, 3, true);
            current = c
        }
        let mut result2 = 0;
        for c in line.chars() {
            result2 += where_numpad(current, c, 26, true);
            current = c
        }
